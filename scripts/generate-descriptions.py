...
Processes all hashtags from fibreguard-v5-data.json and updates hashtag-descriptions.json."""
...
No markdown, no explanation, just the JSON array."""
    ...
    try:
        ...
        return {r["hashtag"]: {"about": r["about"], "fg_score": int(r["fg_score"])} for r in results}
...
def main():
    ...
    # Process ALL hashtags — overwrite existing descriptions for accuracy
    # (known wrong entries like #punch need correction)
    to_process = list(all_items.values())
    
    print(f"Processing {len(to_process)} hashtags in batches of 25...")
    ...
    print("\nSample results:")
    for ht in ["punch", "punchmonkey", "alysaliu", "homedesign", "cleaning"]:
        if ht in descs:
            print(f"  #{ht}: {descs[ht]}")
